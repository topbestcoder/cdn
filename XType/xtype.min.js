...
ig.ua.pixelRatio=window.devicePixelRatio||1;ig.ua.viewport={width:window.innerWidth,height:window.innerHeight};ig.ua.screen={width:window.screen.availWidth*ig.ua.pixelRatio,height:window.screen.availHeight*ig.ua.pixelRatio};ig.ua.iPhone=/iPhone/i.test(navigator.userAgent);ig.ua.iPhone4=(ig.ua.iPhone&&ig.ua.pixelRatio==2);ig.ua.iPad=/iPad/i.test(navigator.userAgent);ig.ua.android=/android/i.test(navigator.userAgent);ig.ua.iOS=ig.ua.iPhone||ig.ua.iPad;ig.ua.winPhone=/IEMobile/i.test(navigator.userAgent);ig.ua.mobile=ig.ua.iOS||ig.ua.android||ig.ua.winPhone;},_initDOMReady:function(){if(ig.modules['dom.ready']){ig._execModules();return;}
...
// lib/impact/font.js
ig.baked=true;ig.module('impact.font').requires('impact.image').defines(function(){"use strict";ig.Font=ig.Image.extend({widthMap:[],indices:[],chars:[],firstChar:32,alpha:1,letterSpacing:1,lineSpacing:0,onload:function(ev){this._loadMetrics(this.data);this.parent(ev);},widthForString:function(text){if(text.indexOf('\n')!==-1){var lines=text.split('\n');var width=0;for(var i=0;i<lines.length;i++){width=Math.max(width,this._widthForLine(lines[i]));}
return width;}
...
x=Math.round(x);y=Math.round(y);if(this.alpha!==1){ig.system.context.globalAlpha=this.alpha;}
...
return a;},update:function(){this.last.x=this.pos.x;this.last.y=this.pos.y;this.pos.x+=this.vel.x*ig.system.tick;this.pos.y+=this.vel.y*ig.system.tick;if(this.currentAnim){this.currentAnim.update();}},getNewVelocity:function(vel,accel,friction,max){if(accel){return(vel+accel*ig.system.tick).limit(-max,max);}
...
if(b.checkAgainst&a.type){b.check(a);}};ig.Entity.solveCollision=function(a,b){var weak=null;if(a.collides==ig.Entity.COLLIDES.LITE||b.collides==ig.Entity.COLLIDES.FIXED){weak=a;}
...
ig.baked=true;ig.module('plugins.impact-splash-loader').requires('impact.loader').defines(function(){ig.ImpactSplashLoader=ig.Loader.extend({endTime:0,fadeToWhiteTime:200,fadeToGameTime:800,logoWidth:340,logoHeight:200,load:function(){this.img=new Image();this.img.src='media/addicting-games-stinger.png';var that=this;this.img.onload=function(){ig.Loader.prototype.load.call(that);}},draw:function(){this._drawStatus+=(this.status-this._drawStatus)/5;var ctx=ig.system.context;var w=ig.system.realWidth;var h=ig.system.realHeight;var scale=w/this.logoWidth/3;var center=(w-this.logoWidth*scale)/2;ctx.fillStyle='#0f0f0f';ctx.fillRect(0,0,w,h);ctx.drawImage(this.img,ig.system.width/2-this.img.width/2,100);ctx.save();ctx.translate(center,h/2.5);ctx.scale(scale,scale);ctx.lineWidth='3';ctx.strokeStyle='rgb(255,255,255)';ctx.strokeRect(25,this.logoHeight+40,300,20);ctx.fillStyle='rgb(255,255,255)';ctx.fillRect(30,this.logoHeight+45,290*this._drawStatus,10);ctx.restore();},drawPaths:function(color,paths){var ctx=ig.system.context;ctx.fillStyle=color;for(var i=0;i<paths.length;i+=2){ctx[ig.ImpactSplashLoader.OPS[paths[i]]].apply(ctx,paths[i+1]);}}});});

// lib/game/entities/particle.js
ig.baked=true;ig.module('game.entities.particle').requires('impact.entity').defines(function(){"use strict";window.EntityParticles=ig.Class.extend({type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,lifetime:5,fadetime:1,_vel:null,_pos:null,vel:{x:0,y:0},image:null,alpha:1,count:10,init:function(x,y,settings){this.count=settings.count||this.count;var l=this.count*2;x-=this.image.width/2;y-=this.image.height/2;this._vel=Array(l);this._pos=Array(l);for(var i=0;i<l;i+=2){this._vel[i]=(Math.random()*2-1)*this.vel.x;this._vel[i+1]=(Math.random()*2-1)*this.vel.y;this._pos[i]=x;this._pos[i+1]=y;}
this.idleTimer=new ig.Timer();},update:function(){if(this.idleTimer.delta()>this.lifetime){ig.game.removeEntity(this);return;}
this.alpha=this.idleTimer.delta().map(this.lifetime-this.fadetime,this.lifetime,1,0);},draw:function(){var l=this.count*2;var p=this._pos,v=this._vel,t=ig.system.tick,ctx=ig.system.context,img=this.image.data,sx=ig.game._rscreen.x,sy=ig.game._rscreen.y;ig.system.context.globalAlpha=this.alpha;for(var i=0;i<l;i+=2){p[i]+=v[i]*t;p[i+1]+=v[i+1]*t;ctx.drawImage(img,p[i]+sx,p[i+1]+sy);}
ig.system.context.globalAlpha=1;}});});

// lib/game/entities/crosshair.js
ig.baked=true;ig.module('game.entities.crosshair').requires('impact.entity').defines(function(){"use strict";window.EntityCrosshair=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/sprites/crosshair.png',18,18),size:{x:2,y:2},offset:{x:8,y:8},type:ig.Entity.TYPE.NONE,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',60,[0]);},update:function(){this.pos.x=ig.input.mouse.x;this.pos.y=ig.input.mouse.y;this.currentAnim.angle-=3*ig.system.tick;}});});

// lib/game/entities/player.js
ig.baked=true;ig.module('game.entities.player').requires('impact.entity','game.entities.particle','game.entities.crosshair').defines(function(){"use strict";window.EntityPlayer=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/sprites/ship.png',24,24),shieldAnimSheet:new ig.AnimationSheet('media/sprites/shield.png',48,48),size:{x:2,y:2},offset:{x:11,y:11},angle:-Math.PI/2,targetAngle:-Math.PI/2,xfriction:{x:800,y:800},maxVel:{x:300,y:300},speed:110,soundShoot:new ig.Sound('media/sounds/plasma-burst.ogg'),soundExplode:new ig.Sound('media/sounds/explosion.ogg'),type:ig.Entity.TYPE.A,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',60,[0]);this.addAnim('shoot',0.05,[3,2,1,0],true);this.shield=new ig.Animation(this.shieldAnimSheet,1,[0]);this.shieldTimer=new ig.Timer(2);this.lastShootTimer=new ig.Timer(0);this.crosshair=ig.game.crosshair;this.soundShoot.volume=0.7;ig.game.player=this;},draw:function(){this.parent();if(this.shieldTimer){this.shield.alpha=this.shieldTimer.delta().map(-0.5,0,0.5,0).limit(0,0.5);this.shield.draw(this.pos.x-24-ig.game._rscreen.x,this.pos.y-24-ig.game._rscreen.y);}},update:function(){if(this.shieldTimer){var d=this.shieldTimer.delta();if(d>0){this.shieldTimer=null;}
else if(d<-1){this.vel.y=d.map(-2,-1,-200,0);this.parent();return;}}
if(this.currentAnim.loopCount>0){this.currentAnim=this.anims.idle;}
if(this.crosshair){this.handleDesktopInput();}
else{this.handleTouchInput();}
this.currentAnim.angle=this.angle+Math.PI/2;this.parent();if(this.pos.x<0){this.pos.x=0;}
else if(this.pos.x>ig.system.width){this.pos.x=ig.system.width;}
if(this.pos.y<0){this.pos.y=0;}
else if(this.pos.y>ig.system.height){this.pos.y=ig.system.height;}},handleDesktopInput:function(){if(ig.input.state('left')){this.vel.x=-this.speed;}
...
this.angle=this.angleTo(this.crosshair);var isShooting=ig.input.state('shoot');if(isShooting&&this.lastShootTimer.delta()>0){this.shoot();this.lastShootTimer.set(0.05);}
if(isShooting&&!this.wasShooting){this.wasShooting=true;this.soundShoot.play();if(!this.soundShoot.currentClip.iloop){this.soundShoot.currentClip.iloop=true;this.soundShoot.currentClip.addEventListener('ended',(function(){this.currentTime=0;this.play();}).bind(this.soundShoot.currentClip),false);}}
else if(this.wasShooting&&!isShooting){this.soundShoot.stop();this.wasShooting=false;}},handleTouchInput:function(){var lstick=ig.game.stickLeft;this.vel.x=lstick.input.x*this.speed;this.vel.y=lstick.input.y*this.speed;var rstick=ig.game.stickRight;if(rstick.amount){this.angle=rstick.angle-Math.PI/2;if(this.lastShootTimer.delta()>0){this.shoot();this.lastShootTimer.set(0.05);}}},kill:function(){this.soundShoot.stop();this.soundExplode.play();ig.game.lastKillTimer.set(0.5);ig.game.spawnEntity(EntityExplosionParticleBlue,this.pos.x,this.pos.y,{count:40});this.pos.y=ig.system.height+300;this.parent();ig.game.loseLive();},shoot:function(){this.currentAnim=this.anims.shoot.rewind();var angle=this.angle+Math.random()*0.1-0.05;ig.game.spawnEntity(EntityPlasma,this.pos.x-1,this.pos.y-1,{angle:angle});}});window.EntityPlasma=ig.Entity.extend({speed:1000,maxVel:{x:1000,y:1000},image:new ig.Image('media/sprites/plasma.png'),size:{x:4,y:4},offset:{x:46,y:46},checkAgainst:ig.Entity.TYPE.B,init:function(x,y,settings){this.parent(x,y,settings);this.vel.x=Math.cos(this.angle)*this.speed;this.vel.y=Math.sin(this.angle)*this.speed;},update:function(){this.pos.x+=this.vel.x*ig.system.tick;this.pos.y+=this.vel.y*ig.system.tick;if(this.pos.x>ig.system.width+200||this.pos.y>ig.system.height+200||this.pos.x<-200||this.pos.y<-200){this.kill();}},draw:function(){ig.system.context.save();ig.system.context.translate(this.pos.x-ig.game._rscreen.x,this.pos.y-ig.game._rscreen.y);ig.system.context.rotate(this.angle+Math.PI/2);ig.system.context.drawImage(this.image.data,-this.offset.x,-this.offset.y);ig.system.context.restore();},check:function(other){if(other instanceof EntityEnemy){other.receiveDamage(10,this);this.kill();}}});window.EntityExplosionParticleBlue=EntityParticles.extend({lifetime:1,fadetime:1,vel:{x:360,y:360},image:new ig.Image('media/sprites/exp-blue.png')});});

// lib/game/menus.js
ig.baked=true;ig.module('game.menus').requires('impact.font').defines(function(){"use strict";window.MenuItem=ig.Class.extend({getText:function(){return'none'},left:function(){},right:function(){},ok:function(){},click:function(){ig.system.canvas.style.cursor='auto';this.ok();}});window.Menu=ig.Class.extend({clearColor:null,name:null,font:new ig.Font('media/fonts/tungsten-48.png'),fontSelected:new ig.Font('media/fonts/tungsten-48-orange.png'),fontTitle:new ig.Font('media/fonts/tungsten-48.png'),current:0,itemClasses:[],items:[],init:function(){this.y=ig.system.height/4+160;for(var i=0;i<this.itemClasses.length;i++){this.items.push(new this.itemClasses[i]());}},update:function(){if(ig.input.pressed('up')){this.current--;}
...
if(hoverItem){ig.system.canvas.style.cursor='pointer';if(ig.input.pressed('shoot')){hoverItem.click();}}
else{ig.system.canvas.style.cursor='auto';}
if(ig.input.pressed('ok')){this.items[this.current].ok();}},draw:function(){if(this.clearColor){ig.system.context.fillStyle=this.clearColor;ig.system.context.fillRect(0,0,ig.system.width,ig.system.height);}
var xs=ig.system.width/2;var ys=this.y;if(this.name){this.fontTitle.draw(this.name,xs,ys-160,ig.Font.ALIGN.CENTER);}
for(var i=0;i<this.items.length;i++){var t=this.items[i].getText();if(i==this.current){this.fontSelected.draw(t,xs,ys,ig.Font.ALIGN.CENTER);}
else{this.font.draw(t,xs,ys,ig.Font.ALIGN.CENTER);}
ys+=this.font.height+20;}}});window.MenuItemSoundVolume=MenuItem.extend({getText:function(){return'Sound Volume: < '+(ig.soundManager.volume*100).round()+'% >';},left:function(){ig.soundManager.volume=(ig.soundManager.volume-0.1).limit(0,1);},right:function(){ig.soundManager.volume=(ig.soundManager.volume+0.1).limit(0,1);},click:function(){if(ig.input.mouse.x>336){this.right();}else{this.left();}}});window.MenuItemMusicVolume=MenuItem.extend({getText:function(){return'Music Volume: < '+(ig.music.volume*100).round()+'% >';},left:function(){ig.music.volume=(ig.music.volume-0.1).limit(0,1);},right:function(){ig.music.volume=(ig.music.volume+0.1).limit(0,1);},click:function(){if(ig.input.mouse.x>336){this.right();}else{this.left();}}});window.MenuItemResume=MenuItem.extend({getText:function(){return'Resume';},ok:function(){ig.game.toggleMenu();}});window.MenuItemBlank=MenuItem.extend({getText:function(){return'';}});window.PauseMenu=Menu.extend({init:function(){if(ig.Sound.enabled){this.itemClasses.push(MenuItemSoundVolume);this.itemClasses.push(MenuItemMusicVolume);}
this.itemClasses.push(MenuItemResume);if(ig.game.mode==XType.MODE.GAME){this.itemClasses.push(MenuItemBlank);this.itemClasses.push(MenuItemBack);}
this.parent();},name:'Menu',clearColor:'rgba(0,0,0,0.9)'});window.MenuItemPlay=MenuItem.extend({getText:function(){return'Start Game!';},ok:function(){ig.game.setGame();}});window.MenuItemScores=MenuItem.extend({getText:function(){return'Highscores';},ok:function(){ig.game.mode=XType.MODE.SCORES;ig.game.menu=new MenuScores();}});window.MenuItemSoundMenu=MenuItem.extend({getText:function(){return'Sound Menu/Pause (ESC Key)';},ok:function(){ig.game.toggleMenu();}});window.TitleMenu=Menu.extend({init:function(){this.itemClasses.push(MenuItemPlay);if(ig.Sound.enabled){this.itemClasses.push(MenuItemSoundMenu);}
if(window.XTypeHighScoreEnabled){this.itemClasses.push(MenuItemScores);}
this.parent();}});window.MenuScores=Menu.extend({loaded:'',mode:'Desktop',init:function(){ig.$('#scores').style.display='block';if(!MenuScores.initialized){ig.$('#scoresBack').onclick=this.cancel.bind(this);ig.$('#showScoresDesktop').onclick=this.loadDesktop.bind(this);ig.$('#showScoresMobile').onclick=this.loadMobile.bind(this);MenuScores.initialized=true;}
this.loadMode(ig.ua.mobile?'Mobile':'Desktop');},loadDesktop:function(){this.loadMode('Desktop');return false;},loadMobile:function(){this.loadMode('Mobile');return false;},loadMode:function(mode){this.mode=mode;ig.$('#showScoresDesktop').className='';ig.$('#showScoresMobile').className='';ig.$('#showScores'+mode).className='active';ig.$('#scoresTable').innerHTML='';ig.$('#scoreNotice').innerHTML='Loading...';ig.game.xhr('scores/index.php',{mode:mode},this.loadCallback.bind(this));},cancel:function(){ig.$('#scores').style.display='none';ig.game.setTitle();return false;},loadCallback:function(scores){if(!scores.length){ig.$('#scoreNotice').innerHTML='No Scores Found.';return;}
var html='<table id="scoresTable"><tr class="head"><td></td><td>Name</td>'+'<td class="score">Score</td><td class="stage">Stage</td><td class="platform">Platform</td></tr>';for(var i=0;i<scores.length;i++){var s=scores[i];html+='<tr>'+'<td class="rank">'+(i+1)+'.</td>'+'<td>'+this.escapeHTML(s.name)+'</a>'+'</td>'+'<td class="score">'+s.score+'</td>'+'<td class="stage">'+s.stage+'</td>'+'<td class="platform">'+s.platform+'</td>'+'</tr>';}
html+='</table>';ig.$('#scoresTable').innerHTML=html;ig.$('#scoreNotice').innerHTML='';},escapeHTML:function(s){return s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');},draw:function(){},update:function(){}});window.MenuScores.initialized=false;window.MenuItemBack=MenuItem.extend({getText:function(){return'Back to Title';},ok:function(){ig.game.setTitle();}});window.GameOverMenu=Menu.extend({init:function(){this.parent();this.y=500;},itemClasses:[MenuItemBack],draw:function(){var ypos=100;this.fontTitle.draw('Game Over',ig.system.width/2,ypos,ig.Font.ALIGN.CENTER);this.fontTitle.draw('Score: '+ig.game.score.zeroFill(6),ig.system.width/2,ypos+60,ig.Font.ALIGN.CENTER);this.parent();}});});

// lib/game/entities/enemy.js
ig.baked=true;ig.module('game.entities.enemy').requires('impact.entity','game.entities.particle').defines(function(){"use strict";window.EntityEnemy=ig.Entity.extend({speed:0,hitTimer:null,dead:false,angle:0,killScore:10,hitScore:10,children:[],parentNode:null,nodeOffset:{x:0,y:0},pivot:{x:0,y:0},maxVel:{x:1000,y:1000},explodeParticles:10,attachmentPoints:[],soundExplode:new ig.Sound('media/sounds/explosion.ogg'),type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,killTimerTime:0.3,init:function(x,y,settings){this.parent(x,y,settings);this.hitTimer=new ig.Timer(0);this.dieTimer=new ig.Timer(0);this.ownAngle=this.angle;},angleToPoint:function(x,y){return Math.atan2(y-(this.pos.y+this.size.y/2),x-(this.pos.x+this.size.x/2));},update:function(){if(!this.isChild){this.vel.x=Math.cos(this.angle)*this.speed;this.vel.y=Math.sin(this.angle)*this.speed;this.parent();if(this.pos.x<-this.image.width||this.pos.x>ig.system.width+10||this.pos.y>ig.system.height+10||this.pos.y<-this.image.height-30){this.kill();}}},attachChild:function(entityClass){var ap=this.attachmentPoints.shift();var c=this.addChild(entityClass,ap.x,ap.y,{angle:(ap.angle*Math.PI)/180});return c;},addChild:function(entityClass,x,y,settings){var c=ig.game.spawnEntity(entityClass,0,0,settings);c.entityType=entityClass;c.nodeOffset.x=x;c.nodeOffset.y=y;c.isChild=true;c.parentNode=this;this.children.push(c);return c;},updateChildren:function(){if(!this.children.length)return;var sv=Math.sin(this.angle-Math.PI/2),cv=Math.cos(this.angle-Math.PI/2);for(var i=0;i<this.children.length;i++){var c=this.children[i];var cx=c.nodeOffset.x,cy=c.nodeOffset.y;c.pos.x=this.pos.x+cv*cx-sv*cy-c.size.x/2+this.size.x/2;c.pos.y=this.pos.y+cv*cy+sv*cx-c.size.y/2+this.size.y/2;c.angle=this.angle+c.ownAngle;c.updateChildren();}},draw:function(){var sx=this.image.width/2,sy=this.image.height/2;ig.system.context.save();ig.system.context.translate(this.pos.x-ig.game._rscreen.x-this.offset.x+sx,this.pos.y-ig.game._rscreen.y-this.offset.y+sy);ig.system.context.rotate(this.angle-Math.PI/2);ig.system.context.drawImage(this.image.data,-sx,-sy);ig.system.context.restore();},receiveDamage:function(amount,other){var childTookDamage=false;if(this.health<=10&&this.children.length){for(var i=0;i<this.children.length;i++){childTookDamage=this.children[i].receiveSilentDamage(amount);if(childTookDamage)break;}}
if(!childTookDamage){this.parent(amount);}
this.hitTimer.set(0.3);ig.game.score+=this.hitScore;if(this.health<=0){this.soundExplode.play();this.explode();ig.game.lastKillTimer.set(this.killTimerTime);ig.game.score+=this.killScore;}
else{var px=other.pos.x-other.size.x/2;var py=other.pos.y-other.size.y/2;ig.game.spawnEntity(EntityExplosionParticleLargeSlow,px,py,{count:1});}},receiveSilentDamage:function(amount){if(this.health<=10&&this.children.length){for(var i=0;i<this.children.length;i++){var childTookDamage=this.children[i].receiveSilentDamage(amount);if(childTookDamage){return true;}}}
else if(this.health>10){this.health-=amount;return true;}
return false;},explode:function(){var px=this.pos.x+this.size.x/2;var py=this.pos.y+this.size.y/2;ig.game.spawnEntity(EntityExplosionParticleLarge,px,py,{count:this.explodeParticles});},kill:function(killedByParent){for(var i=0;i<this.children.length;i++){this.children[i].explode();this.children[i].kill(true);}
if(killedByParent){ig.game.score+=this.killScore;}
if(this.parentNode&&!killedByParent){this.parentNode.children.erase(this);}
this.parent();},check:function(other){if(!other.shieldTimer){other.kill();this.kill();}
else{this.receiveDamage(1000,other);}}});window.EntityExplosionParticleLarge=EntityParticles.extend({lifetime:1,fadetime:1,vel:{x:150,y:150},image:new ig.Image('media/sprites/enemy-explosion.png')});window.EntityExplosionParticleLargeSlow=EntityParticles.extend({lifetime:1,fadetime:1,vel:{x:20,y:20},image:new ig.Image('media/sprites/enemy-explosion.png')});});

// lib/game/entities/enemy-bullet.js
ig.baked=true;ig.module('game.entities.enemy-bullet').requires('game.entities.enemy').defines(function(){"use strict";window.EntityEnemyBullet=EntityEnemy.extend({size:{x:16,y:16},offset:{x:2,y:4},image:new ig.Image('media/sprites/bullet.png'),explodeParticles:3,killTimerTime:0,health:10,speed:170,killScore:0,hitScore:0});});

// lib/game/entities/enemy-heart.js
ig.baked=true;ig.module('game.entities.enemy-heart').requires('game.entities.enemy','game.entities.enemy-bullet').defines(function(){"use strict";window.EntityEnemyHeart=EntityEnemy.extend({size:{x:64,y:64},offset:{x:0,y:12},image:new ig.Image('media/sprites/heart.png'),health:400,bullets:16,killScore:10000,moveTarget:{x:0,y:0},angleTarget:{x:0,y:0},explodeParticles:40,killTimerTime:0.3,attachmentPoints:[{x:40,y:42,angle:-45},{x:44,y:-20,angle:-110}],init:function(x,y,settings){this.parent(x,y-18,settings);this.shootTimer=new ig.Timer(2);this.angle=Math.PI/2;this.moveTimer=new ig.Timer(4);this.angleTarget={x:ig.system.width/2,y:ig.system.height/2};this.moveTarget={x:ig.system.width/2,y:ig.system.height/6};},speed:20,maxVel:{x:60,y:70},friction:{x:20,y:20},update:function(){if(this.moveTimer.delta()>0){this.moveTarget={x:(Math.random().map(0,1,ig.system.width/3,ig.system.width-ig.system.width/3)),y:(Math.random().map(0,1,ig.system.height/8,ig.system.height/6))};this.moveTimer.set(Math.random()*6+12);this.angleTarget={x:ig.game.player.pos.x,y:ig.game.player.pos.y};}
var a=(this.angle-this.angleToPoint(this.angleTarget.x,this.angleTarget.y))*ig.system.tick;this.angle-=a;if(Math.abs(this.pos.x-this.moveTarget.x)>20){this.accel.x=(this.pos.x-this.moveTarget.x)<0?this.speed:-this.speed;}
else{this.accel.x=0;}
if(Math.abs(this.pos.y-this.moveTarget.y)>20){this.accel.y=(this.pos.y-this.moveTarget.y)<0?this.speed:-this.speed;}
else{this.accel.y=0;}
this.last.x=this.pos.x;this.last.y=this.pos.y;this.vel.x=this.getNewVelocity(this.vel.x,this.accel.x,this.friction.x,this.maxVel.x);this.vel.y=this.getNewVelocity(this.vel.y,this.accel.y,this.friction.y,this.maxVel.y);this.pos.x+=this.vel.x*ig.system.tick;this.pos.y+=this.vel.y*ig.system.tick;if(this.children.length==0&&this.shootTimer.delta()>0){var inc=140/(this.bullets-1);var a2=20;var radius=22;for(var i=0;i<this.bullets;i++){var angle=a2*Math.PI/180;var x=this.pos.x+24+Math.cos(angle)*radius;var y=this.pos.y+44+Math.sin(angle)*radius;ig.game.spawnEntity(EntityEnemyBullet,x,y,{angle:angle});a2+=inc;}
this.shootTimer.reset();}
this.updateChildren();},kill:function(){this.parent();ig.game.spawnEntity(EntityExplosionHuge,this.pos.x,this.pos.y);ig.game.heart=null;}});window.EntityExplosionHuge=ig.Entity.extend({lifetime:1,fadetime:1,alpha:0,img:new ig.Image('media/sprites/explosion-huge.jpg',512,512),init:function(x,y,settings){this.parent(x,y,settings);this.idleTimer=new ig.Timer();},update:function(){if(this.idleTimer.delta()>this.lifetime){this.kill();return;}
...
ig.baked=true;ig.module('game.entities.enemy-missilebox').requires('game.entities.enemy','game.entities.enemy-bullet').defines(function(){"use strict";window.EntityEnemyMissilebox=EntityEnemy.extend({size:{x:44,y:44},offset:{x:2,y:2},image:new ig.Image('media/sprites/missilebox.png'),health:120,bullets:8,reloadTime:3,explodeParticles:10,killScore:300,init:function(x,y,settings){this.parent(x,y-18,settings);this.moveTimer=new ig.Timer();this.angle=Math.PI/2;this.startAngle=this.ownAngle;this.shootTimer=new ig.Timer(Math.random()*this.reloadTime*2);},update:function(){this.parent();if(this.shootTimer.delta()>0){var inc=140/(this.bullets-1);var a=20+(this.angle-Math.PI/2)*180/Math.PI;var radius=22;for(var i=0;i<this.bullets;i++){var angle=a*Math.PI/180;var x=this.pos.x+20+Math.cos(angle)*radius;var y=this.pos.y+20+Math.sin(angle)*radius;ig.game.spawnEntity(EntityEnemyBullet,x,y,{angle:angle});a+=inc;}
this.shootTimer.set(this.reloadTime);}}});});

// lib/game/entities/enemy-plasmabox.js
ig.baked=true;ig.module('game.entities.enemy-plasmabox').requires('game.entities.enemy','game.entities.enemy-bullet').defines(function(){"use strict";window.EntityEnemyPlasmabox=EntityEnemy.extend({size:{x:44,y:44},offset:{x:2,y:2},image:new ig.Image('media/sprites/plasmabox.png'),health:180,reloadTime:4,bullets:32,explodeParticles:10,killScore:400,init:function(x,y,settings){this.parent(x,y-18,settings);this.moveTimer=new ig.Timer();this.angle=Math.PI/2;this.startAngle=this.ownAngle;this.shootTimer=new ig.Timer(Math.random()*this.reloadTime*2);},update:function(){this.parent();if(this.shootTimer.delta()>0){var inc=360/(this.bullets-1);var a=0;var radius=0;for(var i=0;i<this.bullets;i++){var angle=a*Math.PI/180;var x=this.pos.x+18;var y=this.pos.y+18;ig.game.spawnEntity(EntityEnemyPlasmaBullet,x,y,{angle:angle});a+=inc;}
this.shootTimer.set(this.reloadTime);}}});window.EntityEnemyPlasmaBullet=EntityEnemy.extend({size:{x:8,y:8},offset:{x:28,y:28},image:new ig.Image('media/sprites/pbullet.png'),health:10,speed:10,maxSpeed:160,type:ig.Entity.TYPE.NONE,update:function(){this.speed=Math.min(this.maxSpeed,this.speed+ig.system.tick*100);this.vel.x=Math.cos(this.angle)*this.speed;this.vel.y=Math.sin(this.angle)*this.speed;this.pos.x+=this.vel.x*ig.system.tick;this.pos.y+=this.vel.y*ig.system.tick;if(this.pos.x>ig.system.width+200||this.pos.y>ig.system.height+200||this.pos.x<-200||this.pos.y<-200){this.kill();}},draw:function(){ig.system.context.drawImage(this.image.data,this.pos.x-ig.game._rscreen.x-this.offset.x,this.pos.y-ig.game._rscreen.y-this.offset.y);}});});

// lib/game/entities/enemy-arm.js
ig.baked=true;ig.module('game.entities.enemy-arm').requires('game.entities.enemy','game.entities.enemy-bullet').defines(function(){"use strict";window.EntityEnemyArm=EntityEnemy.extend({size:{x:44,y:44},offset:{x:2,y:2},image:new ig.Image('media/sprites/arm.png'),health:70,killScore:50,explodeParticles:5,attachmentPoints:[{x:-8,y:42,angle:20},{x:32,y:6,angle:-70},{x:-32,y:6,angle:70}],init:function(x,y,settings){this.parent(x,y-18,settings);this.moveTimer=new ig.Timer();this.angle=Math.PI/2;this.startAngle=this.ownAngle;},update:function(){this.parent();this.ownAngle=this.startAngle+Math.cos(this.moveTimer.delta())*0.05;}});});

// lib/plugins/analog-stick.js
ig.baked=true;ig.module('plugins.analog-stick').requires('impact.system').defines(function(){ig.AnalogStick=ig.Class.extend({stickSize:30,baseSize:70,stickColor:'rgba(255,255,255,0.3)',baseColor:'rgba(255,255,255,0.3)',pos:{x:0,y:0},input:{x:0,y:0},pressed:false,angle:0,amount:0,_touchId:null,init:function(x,y,baseSize,stickSize){this.pos={x:x,y:y};this.baseSize=baseSize||this.baseSize;this.stickSize=stickSize||this.stickSize;this.max=this.baseSize-this.stickSize/3;ig.system.canvas.addEventListener('touchstart',this.touchStart.bind(this),false);document.addEventListener('touchmove',this.touchMove.bind(this),false);document.addEventListener('touchend',this.touchEnd.bind(this),false);ig.input.isUsingMouse=true;},touchStart:function(ev){ev.preventDefault();if(this.pressed){return;}
for(var i=0;i<ev.touches.length;i++){var touch=ev.touches[i];var ip=ig.input;ip.mouse.x=touch.pageX*ig.internalScale;ip.mouse.y=touch.pageY*ig.internalScale;ip.actions['shoot']=true;if(!ip.locks['shoot']){ip.presses['shoot']=true;ip.locks['shoot']=true;}
var xd=this.pos.x-touch.pageX*ig.internalScale;var yd=this.pos.y-touch.pageY*ig.internalScale;if(Math.sqrt(xd*xd+yd*yd)<this.baseSize){this.pressed=true;this._touchId=touch.identifier;this._moved(touch);return;}}},touchMove:function(ev){ev.preventDefault();for(var i=0;i<ev.changedTouches.length;i++){if(ev.changedTouches[i].identifier==this._touchId){this._moved(ev.changedTouches[i]);return;}}},_moved:function(touch){var x=touch.pageX*ig.internalScale-this.pos.x;var y=touch.pageY*ig.internalScale-this.pos.y;this.angle=Math.atan2(x,-y);this.amount=Math.min(1,Math.sqrt(x*x+y*y)/this.max);this.input.x=Math.sin(this.angle)*this.amount;this.input.y=-Math.cos(this.angle)*this.amount;},touchEnd:function(ev){ig.input.delayedKeyup['shoot']=true;for(var i=0;i<ev.changedTouches.length;i++){if(ev.changedTouches[i].identifier==this._touchId){this.pressed=false;this.input.x=0;this.input.y=0;this.amount=0;this._touchId=null;return;}}},draw:function(){var ctx=ig.system.context;ctx.beginPath();ctx.arc(this.pos.x,this.pos.y,this.baseSize,0,(Math.PI*2),true);ctx.lineWidth=3;ctx.strokeStyle=this.baseColor;ctx.stroke();ctx.beginPath();ctx.arc(this.pos.x+this.input.x*this.max,this.pos.y+this.input.y*this.max,this.stickSize,0,(Math.PI*2),true);ctx.fillStyle=this.stickColor;ctx.fill();}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','plugins.impact-splash-loader','game.entities.player','game.menus','game.entities.enemy-heart','game.entities.enemy-missilebox','game.entities.enemy-plasmabox','game.entities.enemy-arm','plugins.analog-stick').defines(function(){"use strict";Number.zeroes='000000000000';Number.prototype.zeroFill=function(d){var s=this.toString();return Number.zeroes.substr(0,d-s.length)+s;};window.XType=ig.Game.extend({menu:null,mode:0,font:new ig.Font('media/fonts/tungsten-48.png'),fontSmall:new ig.Font('media/fonts/tungsten-18.png'),backdrop:new ig.Image('media/background/backdrop.png'),grid:new ig.Image('media/background/grid.png'),music:new ig.Sound('media/music/xtype.ogg',false),title:new ig.Image('media/xtype-title.png'),pauseButton:new ig.Image('media/pause-button.png'),madeWithImpact:new ig.Image('media/made-with-impact.png'),addictingGames:new ig.Image('media/addicting-games.png'),instructions:new ig.Image('media/instructions-'+
(ig.ua.mobile?'mobile':'desktop')+'.png'),score:0,lives:3,level:{level:0,support:1,plasma:0,missile:0},stickLeft:null,stickRight:null,init:function(){var bgmap=new ig.BackgroundMap(620,[[1]],this.grid);bgmap.repeat=true;this.backgroundMaps.push(bgmap);if(!ig.ua.mobile){ig.input.bind(ig.KEY.MOUSE1,'shoot');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.W,'up');ig.input.bind(ig.KEY.S,'down');ig.input.bind(ig.KEY.A,'left');ig.input.bind(ig.KEY.D,'right');ig.input.bind(ig.KEY.ENTER,'ok');ig.input.bind(ig.KEY.ESC,'menu');ig.music.volume=0.6;ig.music.add(this.music);}
else{var radius=60;var margin=20;var y=ig.system.height-radius-margin;var x1=radius+margin;var x2=ig.system.width-radius-margin;this.stickLeft=new ig.AnalogStick(x1,y,radius,30);this.stickRight=new ig.AnalogStick(x2,y,radius,30);}
this.reset();this.setTitle();XType.initialized=true;},reset:function(){this.heart=null;this.lastKillTimer=new ig.Timer(-2);this.entities=[];this.entitiesSortedByPosTypeA=[];this.entitiesSortedByPosTypeB=[];this.score=0,this.lives=3,this.level={level:0,support:1,plasma:0,missile:0};},setGame:function(){window.scrollTo(0,0);ig.music.play();ig.system.canvas.style.cursor='';this.menu=null;this.initTimer=new ig.Timer(3);this.lastKillTimer.reset();if(!ig.ua.mobile){this.crosshair=this.spawnEntity(EntityCrosshair,0,0);}
this.bossEndTimer=null;this.player=this.spawnEntity(EntityPlayer,ig.system.width/2,ig.system.height+24);this.mode=XType.MODE.GAME;},setTitle:function(){this.reset();this.mode=XType.MODE.TITLE;this.menu=new TitleMenu();if(window.XTypeHighScoreEnabled){ig.$('#scoreBox').style.display='none';}},setGameOver:function(){if(this.score>0&&window.XTypeHighScoreEnabled){var name=this.getCookie('scoreName');if(name){ig.$('#scoreName').value=name;}
ig.$('#scoreBox').style.display='block';ig.$('#scoreForm').style.display='block';ig.$('#scoreResponse').style.display='none';}
this.mode=XType.MODE.GAME_OVER;this.menu=new GameOverMenu();},toggleMenu:function(){if(this.mode==XType.MODE.TITLE){if(this.menu instanceof TitleMenu){this.menu=new PauseMenu();}
else{this.menu=new TitleMenu();}}
else{if(this.menu){ig.system.canvas.style.cursor='';this.menu=null;}
else{this.menu=new PauseMenu();}}},checkBoss:function(){if(!this.heart&&!this.initTimer){if(!this.bossEndTimer){this.bossEndTimer=new ig.Timer(2);}
else if(this.bossEndTimer&&this.bossEndTimer.delta()>0){this.bossEndTimer=null;this.spawnBoss();}}},spawnBoss:function(){this.heart=this.spawnEntity(EntityEnemyHeart,ig.system.width/2,0);this.level.level+=1;this.level.support+=1;this.level.plasma+=this.level.level%2?0:1;this.level.missile+=this.level.level%2?1:0;for(var i=0;i<this.level.support;i++){this.spawEntityRandom(EntityEnemyArm);}
for(i=0;i<this.level.missile;i++){this.spawEntityRandom(EntityEnemyMissilebox);}
for(i=0;i<this.level.plasma;i++){this.spawEntityRandom(EntityEnemyPlasmabox);}
this.mirrorChildren(this.heart,this.heart);this.heart.update();var ents=this.getEntitiesByType(EntityEnemyArm);var maxY=0;for(i=0;i<ents.length;i++){maxY=Math.max(ents[i].pos.y,maxY);}
this.heart.pos.y=-maxY-120;this.heart.vel.y=70;this.heart.update();},mirrorChildren:function(src,dest){var l=src.children.length
for(var i=0;i<l;i++){var srcEnt=src.children[i];var settings={angle:-srcEnt.ownAngle};var destEnt=dest.addChild(srcEnt.entityType,-srcEnt.nodeOffset.x,srcEnt.nodeOffset.y,settings);this.mirrorChildren(srcEnt,destEnt);}},spawEntityRandom:function(type){var ents=this.getEntitiesByType(EntityEnemyArm);ents.push(this.heart);for(var i=0;i<20;i++){var e=ents.random();if(!e.attachmentPoints||!e.attachmentPoints.length){continue;}
if(type==EntityEnemyArm&&e instanceof EntityEnemyArm&&e.attachmentPoints.length!=3&&e.parentNode instanceof EntityEnemyHeart){continue;}
e.attachChild(type);return;}},update:function(){if(!this.menu&&(ig.input.pressed('menu')||(ig.ua.mobile&&ig.input.pressed('shoot')&&ig.input.mouse.x<100&&ig.input.mouse.y<100))){this.toggleMenu();}
if(this.menu){this.backgroundMaps[0].scroll.y-=100*ig.system.tick;this.menu.update();if(this.mode==XType.MODE.TITLE&&ig.input.pressed('shoot')&&ig.input.mouse.x>ig.system.width-154&&ig.input.mouse.y>ig.system.height-56){window.top.location.href='http://www.addictinggames.com/';}
if(!(this.menu instanceof GameOverMenu)){return;}}
this.parent();this.backgroundMaps[0].scroll.y-=100*ig.system.tick;if(this.mode==XType.MODE.GAME){this.checkBoss();}},loseLive:function(){this.lives--;if(this.lives>0){this.player=this.spawnEntity(EntityPlayer,ig.system.width/2,ig.system.height+24);this.livesRemainingTimer=new ig.Timer(2);}
else{this.setGameOver();}},draw:function(){this.backdrop.draw(0,0);var d=this.lastKillTimer.delta();ig.system.context.globalAlpha=d<0?d*-3+0.3:0.3;for(var i=0;i<this.backgroundMaps.length;i++){this.backgroundMaps[i].draw();}
ig.system.context.globalAlpha=1;if(d<0.5){this._rscreen.x=Math.random()*10*(d-0.5);this._rscreen.y=Math.random()*10*(d-0.5);}
else{this._rscreen.x=this._rscreen.y=0;}
ig.system.context.globalCompositeOperation='lighter';for(var i=0;i<this.entities.length;i++){this.entities[i].draw();}
ig.system.context.globalCompositeOperation='source-over';if(this.mode==XType.MODE.GAME){this.drawUI();}
else if(this.mode==XType.MODE.TITLE){this.drawTitle();}
if(this.menu){this.menu.draw();}},drawUI:function(){if(ig.ua.mobile){this.stickLeft.draw();this.stickRight.draw();this.pauseButton.draw(16,10);}
this.font.draw(this.score.zeroFill(6),ig.system.width-32,32,ig.Font.ALIGN.RIGHT);if(this.bossEndTimer){var d=-this.bossEndTimer.delta();var a=d>1.7?d.map(2,1.7,0,1):d<1?d.map(1,0,1,0):1;var xs=ig.system.width/2;var ys=ig.system.height/3+(d<1?Math.cos(1-d).map(1,0,0,250):0);var b=this.level.level;this.font.alpha=a;this.font.draw('Stage '+b+' Clear',xs,ys,ig.Font.ALIGN.CENTER);this.font.alpha=1;}
if(this.livesRemainingTimer){var d2=-this.livesRemainingTimer.delta();var a2=d2>1.7?d2.map(2,1.7,0,1):(d2<1?d2:1);var xs2=ig.system.width/2;var ys2=ig.system.height/3+(d2<1?Math.cos(1-d2).map(1,0,0,250):0);this.font.alpha=Math.max(a2,0);if(this.lives>1){this.font.draw(this.lives+' Ships Remaining',xs2,ys2,ig.Font.ALIGN.CENTER);}
else{this.font.draw(this.lives+' Ship Remaining',xs2,ys2,ig.Font.ALIGN.CENTER);}
this.font.alpha=1;if(d2<0){this.livesRemainingTimer=null;}}
if(this.initTimer){var initTime=this.initTimer.delta();if(initTime>0){this.initTimer=null;this.spawnBoss();}
ig.system.context.globalAlpha=initTime.map(-0.5,0,1,0).limit(0,1);if(ig.ua.mobile){this.instructions.draw(100,ig.system.height-210);}
else{this.instructions.draw(25,260);}
ig.system.context.globalAlpha=1;}},drawTitle:function(){var xs=ig.system.width/2;var ys=ig.system.height/4;this.title.draw(96,96);var xc=8;var yc=ig.system.height-40;ig.system.context.globalAlpha=0.6;ig.system.context.globalAlpha=1;this.addictingGames.draw(ig.system.width-154,ig.system.height-56);},entitiesSortedByPosTypeA:[],entitiesSortedByPosTypeB:[],sortByYPos:function(a,b){return a.pos.y-b.pos.y;},sortByYPosSize:function(a,b){return(a.pos.y+a.size.y)-(b.pos.y+b.size.y);},spawnEntity:function(type,x,y,settings){var entityClass=typeof(type)==='string'?ig.global[type]:type;if(!entityClass){throw("Can't spawn entity of type "+type);}
var ent=new(entityClass)(x,y,settings||{});this.entities.push(ent);if(ent.name){this.namedEntities[ent.name]=ent;}
if(ent.type||ent.checkAgainst){if(ent.type==ig.Entity.TYPE.A||(ent.checkAgainst&ig.Entity.TYPE.B)){this.entitiesSortedByPosTypeA.push(ent);}
else{this.entitiesSortedByPosTypeB.push(ent);}}
return ent;},removeEntity:function(ent){if(ent.name){delete this.namedEntities[ent.name];}
if(ent.type||ent.checkAgainst){if(ent.type==ig.Entity.TYPE.A||(ent.checkAgainst&ig.Entity.TYPE.B)){this.entitiesSortedByPosTypeA.erase(ent);}
else{this.entitiesSortedByPosTypeB.erase(ent);}}
ent._killed=true;ent.checkAgainst=ig.Entity.TYPE.NONE;ent.collides=ig.Entity.COLLIDES.NEVER;this._deferredKill.push(ent);},checkEntities:function(){var seB=this.entitiesSortedByPosTypeA;var seA=this.entitiesSortedByPosTypeB;seA.sort(this.sortByYPosSize);seB.sort(this.sortByYPos);var c1=0,c2=0;var k=0,e1=null,e2=null,my=0,noskip=true;for(var i=0;i<seA.length;i++){e1=seA[i];noskip=true;my=e1.pos.y+e1.size.y;for(var j=k;j<seB.length&&(e2=seB[j])&&(e2.pos.y<my);j++){if(noskip&&e2.pos.y+e2.size.y<e1.pos.y){k=j;}
else{noskip=false;}
if(e1.touches(e2)){ig.Entity.checkPair(e1,e2);}}}},submitScore:function(){var name=ig.$('#scoreName').value;if(!name)return;ig.$('#scoreName').blur();ig.$('#scoreForm').style.display='none';ig.$('#scoreResponse').style.display='block';ig.$('#scoreResponse').innerHTML='Sending...';this.setCookie('scoreName',name,100);var so={stage:this.level.level,score:Math.floor(this.score),name:name};so.sh=(so.score+so.stage)^0x8d525a2f;this.xhr('scores/index.php',so,this.scoreResponse.bind(this));},scoreResponse:function(data){if(data.success){ig.$('#scoreResponse').innerHTML='Your Rank: #'+data.rank;}
else{ig.$('#scoreResponse').innerHTML='Failed. Sorry.';}},xhr:function(url,data,callback){var post=[];if(data){for(var key in data){post.push(key+'='+encodeURIComponent(data[key]));}}
...
xhr.open('POST',url);xhr.setRequestHeader("Content-type","application/x-www-form-urlencoded");xhr.send(postString);},setCookie:function(name,value,days){days=days||1;var expire=new Date();expire.setTime(Date.now()+3600000*24*days);document.cookie=name+"="+escape(value)
+";expires="+expire.toGMTString();},getCookie:function(name){var re=new RegExp('[; ]'+name+'=([^\\s;]*)');var match=(' '+document.cookie).match(re);if(name&&match){return unescape(match[1]);}
else{return null;}}});window.XType.MODE={TITLE:0,GAME:1,GAME_OVER:2,SCORES:3};window.XType.paused=false;window.XType.startGame=function(){ig.Sound.channels=2;ig.System.drawMode=ig.System.DRAW.SUBPIXEL;var width=480;var height=720;if(ig.ua.mobile){ig.Sound.enabled=false;var wpw=window.innerWidth*ig.ua.pixelRatio;var wph=window.innerHeight*ig.ua.pixelRatio;var scale=width/wpw;height=wph*scale;ig.internalScale=scale*ig.ua.pixelRatio;var canvas=ig.$('#canvas');canvas.style.width=Math.floor(window.innerWidth)+'px';canvas.style.height=Math.floor(window.innerHeight)+'px';if(window.XTypeHighScoreEnabled){ig.$('#scoreBox').style.width=Math.floor(window.innerWidth)+'px';ig.$('#scoreBox').style.top=(240/ig.internalScale)+'px';ig.$('#scores').style.width=(410/ig.internalScale)+'px';ig.$('#scores').style.height=Math.floor(window.innerHeight)+'px';}}
else{ig.$('#canvas').className='desktop';if(window.XTypeHighScoreEnabled){ig.$('#scoreBox').style.bottom=0;ig.$('#scores').style.bottom=0;}}
if(window.XTypeHighScoreEnabled){ig.$('#scoreForm').onsubmit=function(){if(ig.game){ig.game.submitScore();}
return false;}}
ig.main('#canvas',XType,60,width,height,1,ig.ImpactSplashLoader);};window.XType.checkOrientation=function(){var isPortrait=XType.isPortrait();if(isPortrait===XType.wasPortrait){return;}
XType.wasPortrait=isPortrait;ig.$('#loading').style.display='none';if(isPortrait){ig.$('#canvas').style.display='block';ig.$('#rotate').style.display='none';if(XType.initialized&&XType.paused){ig.system.startRunLoop();XType.paused=false;}
else if(!XType.initialized){window.scrollTo(0,0);window.setTimeout(XType.startGame,1);}}
else{if(XType.initialized){ig.system.stopRunLoop();XType.paused=true;}
ig.$('#canvas').style.display='none';ig.$('#rotate').style.display='block';}};window.XType.wasPortrait=-1;window.XType.isPortrait=function(){return(!ig.ua.mobile||window.innerHeight>window.innerWidth);};window.addEventListener('orientationchange',XType.checkOrientation,false);window.addEventListener('resize',XType.checkOrientation,false);window.XType.checkOrientation();});